...
public class ExpConstructeur {

    public static double getExp(UUID uuid, Player player) {
        double exp_constructeur = 0.0;
        try {
            ...
            if(resultSet.next()) {
                ...
                exp_constructeur = resultSet.getDouble("exp_constructeur");
            }
            connection.close();
    ...
    public static void addExp(UUID uuid, Player player, double exp) {
        try {
            final Connection connection = DatabaseManager.JOBS.getDatabaseAccess().getConnection();
            final PreparedStatement preparedStatement = connection.prepareStatement("UPDATE expjobs SET exp_constructeur = ? WHERE uuid = ?");
