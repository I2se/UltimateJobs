...
public class ExpChasseur {
    ...
    public static double getExp(UUID uuid, Player player) {
        ...
        try {
            ...
            if(resultSet.next()) {
                ...
                exp_chasseur = resultSet.getDouble("exp_chasseur");
            }
            connection.close();
    ...
    public static void addExp(UUID uuid, Player player, double exp) {
        try {
            final Connection connection = DatabaseManager.JOBS.getDatabaseAccess().getConnection();
            final PreparedStatement preparedStatement = connection.prepareStatement("UPDATE expjobs SET exp_chasseur = ? WHERE uuid = ?");
