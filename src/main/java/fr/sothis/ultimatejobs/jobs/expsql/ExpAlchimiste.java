...
public class ExpAlchimiste {

    public static double getExp(UUID uuid, Player player) {
        double exp_alchimiste = 0.0;
        try {
            ...
            if(resultSet.next()) {
                ...
                exp_alchimiste = resultSet.getDouble("exp_alchimiste");
            }
            connection.close();
    ...
    public static void addExp(UUID uuid, Player player, double exp) {
        try {
            final Connection connection = DatabaseManager.JOBS.getDatabaseAccess().getConnection();
            final PreparedStatement preparedStatement = connection.prepareStatement("UPDATE expjobs SET exp_alchimiste = ? WHERE uuid = ?");
