...
public class ExpExplorateur {

    public static double getExp(UUID uuid, Player player) {
        double exp_explorateur = 0.0;
        try {
            ...
            if(resultSet.next()) {
                ...
                exp_explorateur = resultSet.getDouble("exp_explorateur");
            }
            connection.close();
    ...
    public static void addExp(UUID uuid, Player player, double exp) {
        try {
            final Connection connection = DatabaseManager.JOBS.getDatabaseAccess().getConnection();
            final PreparedStatement preparedStatement = connection.prepareStatement("UPDATE expjobs SET exp_explorateur = ? WHERE uuid = ?");
