...
public class ExpEnchanteur {

    public static double getExp(UUID uuid, Player player) {
        double exp_enchanteur = 0.0;
        try {
            ...
            if(resultSet.next()) {
                ...
                exp_enchanteur = resultSet.getDouble("exp_enchanteur");
            }
            connection.close();
    ...
    public static void addExp(UUID uuid, Player player, double exp) {
        try {
            final Connection connection = DatabaseManager.JOBS.getDatabaseAccess().getConnection();
            final PreparedStatement preparedStatement = connection.prepareStatement("UPDATE expjobs SET exp_enchanteur = ? WHERE uuid = ?");
