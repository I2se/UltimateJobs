    ...
    public static int getLevel(UUID uuid, Player player) {
        ...
        try {
            ...
            if(resultSet.next()) {
                ...
                level_chasseur = resultSet.getInt("level_chasseur");
            }
            connection.close();
        } catch(SQLException throwables) {
            throwables.printStackTrace();
        }
        return level_chasseur;
    }

    public static void setLevel(UUID uuid, Player player, int level) {
        try {
            final Connection connection = DatabaseManager.JOBS.getDatabaseAccess().getConnection();
            final PreparedStatement preparedStatement = connection.prepareStatement("UPDATE leveljobs SET level_chasseur = ? WHERE uuid = ?");
